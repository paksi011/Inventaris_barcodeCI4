...
class Masuk_model extends Model
{
    ...
    public function getBarangMasukLebihDariSeminggu()
    {
        $this->db = \Config\Database::connect();
        $date = date('Y-m-d', strtotime('-1 week'));
        return $this->where('tgl_pembelian <=', $date)->findAll();
    }

    public function deleteBarangMasuk($id_pengadaan)
    {
        $this->db = \Config\Database::connect();
        $this->delete($id_pengadaan);
    }
}
